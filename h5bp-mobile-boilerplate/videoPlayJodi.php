 <?
  $title = "Jodi Video"; // this is for <title>
  //$addCSS = ""; //custom CSS for this page only
  include_once("inc/header.php");
?>

 <!--start typing html/body text here -->

<script src="js/AC_QuickTime.js"></script>
<script src ="js/videoFunc.js"></script>


<div id="container">


<script language="javascript" type="text/javascript">
	QT_WriteOBJECT('videos/iPhone4_Jodi-WIREDRIVE.mov' , '320', '460', '', 'EnableJavaScript', 'True', 'scale', 'aspect', 'Autoplay', 'true', 'controller', 'false',
	'postdomevents', 'True', 'emb#NAME' , 'movie1' , 'obj#id' , 'movie1', 'emb#id',
	'qtmovie_embed') ;
</script>

 




<?php include_once("inc/footer.php");?> 